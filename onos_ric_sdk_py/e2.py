...
class Subscription(aiomsa.e2.Subscription):
    def __init__(
        self,
        subscription_id: str,
        app_id: str,
...
class E2Client(aiomsa.e2.E2Client):
    def __init__(
        self,
        app_id: str,
        e2t_endpoint: str,
    ...
    async def list_nodes(self, oid: Optional[str] = None) -> List[aiomsa.e2.E2Node]:
        nodes = []
    ...
    async def control(
        ...
        header: bytes,
        message: bytes,
        control_ack_request: aiomsa.e2.ControlAckRequest,
    ) -> Optional[bytes]:
        ...
        if value is None:
            raise Exception("E2 control response is missing")
        if name == "control_failure":
            raise Exception(f"Control failure ({value.cause}): {value.message}")
                    ...
                    Action(
                        ...
                        payload=(
                            ...
                            if action.data is None
                                ...
                                encoding=Encoding.ENCODING_PROTO, data=action.data
                            )
                        ),
                        subsequent_action=SubsequentAction(
                            type=SubsequentActionType(action.subsequent_action_type),
                            time_to_wait=TimeToWait(action.time_to_wait),
                        ),
                    )
